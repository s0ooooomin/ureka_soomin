...
// Spring Fromeword 역할 - 우리가 사용한 annotation, 지정한 속성값 저장
// AboutMe annotation 처리
public class Test {
	public static void main(String[] args) throws Exception {
		Class<?> myClass = Class.forName("com.mycom.myapp.annotation.MyClass");
		Annotation[] annotations = myClass.getAnnotations();
		
		// annotations에서 받은 annotation 하나하나에 대해
		for (Annotation annotation : annotations) {
			...
			if (annotation instanceof AboutMe2) {
				AboutMe aboutMe = (AboutMe) annotation;
				System.out.println(aboutMe.love());
				System.out.println(aboutMe.hate());
			}
			
			if (annotation instanceof AboutMe3) {
				AboutMe aboutMe = (AboutMe) annotation;
				System.out.println(aboutMe.love());
				System.out.println(aboutMe.hate());
